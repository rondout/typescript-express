...
// 处理好友申请
friendRequestRouter.post("/handle", async (req, res) => {
  const params = req.body as HandleFriendsRequestParams;
  const result = await friendRequestService.handleRequest(params);
  res.send(result);
});
